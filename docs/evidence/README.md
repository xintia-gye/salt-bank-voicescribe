...
| 0 | [`09_new_pipeline_run_output.txt`](09_new_pipeline_run_output.txt) | **A brand-new Lakeflow pipeline was created, run (dev), and its output captured** — pipeline `voicescribe-medallion-demo` (`6010b3d7-…`), update `3bd8e45b` → `COMPLETED`; full run trace + layer counts (40/40/40/40) + category distribution + 100% accuracy + sample Gold rows, all in schema `voicescribe_pipeline_demo`. |
| ★ | [`10_end_to_end_execution.txt`](10_end_to_end_execution.txt) | **All three reviewer-requested proofs in one plain-text file** — **[A]** pipeline run with bronze/silver/gold row counts (40/40/40/40, 100% accuracy), **[B]** a sample transcript → structured summary the models produced (`CALL-ed7b057e28`), and **[C]** a real Genie conversation (question → generated SQL → returned rows), captured live via the Genie Conversation API. |
| ★ | [`11_notebook_execution_proof.txt`](11_notebook_execution_proof.txt) | **Notebook execution proof, cell by cell** — the two evidence notebooks run on Databricks as jobs (`221692384572973`, `153107346629968`), showing each cell's code (`IN`) and the output Databricks produced (`OUT`): pipeline `COMPLETED`, 40 rows/layer, RO transcript → EN summary + ticket, 100% accuracy. Extracted from the executed notebooks. |
